...
function Fetch() {
  co(function*() {
    ...
    let db = yield MongoClient.connect(config.mongodb);
    console.log('[event] database connection established');
    let res = yield request('http://www3.nbswz.com.cn:8080/swz/Shared?&/getTideWater')
    ...
    yield db.collection('nbswz').insertMany(res);
